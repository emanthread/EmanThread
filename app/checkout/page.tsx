                        ...
                        <div key={item.product.id} className={cn("flex items-start gap-3 pb-3 border-b border-border last:border-0 transition-opacity", !isSelected && "opacity-50")}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {(() => {
                              ...
                              const hasMaleShalwar = !!((profile as any).shalwar1 || (profile as any).shalwarLength1);
                              const hasFemaleSimpleShalwar = !!(profile as any).ladSimpleShalwar1;
                              const hasFemaleBeltShalwar = !!(profile as any).ladShalwarBelt1;
                              const hasTrouser = !!((profile as any).trouserdata1 || (profile as any).trouserLength1);
                    ...
                    {hasStitchingSelected && (
                      <div className="mt-3 p-3 rounded-lg border border-primary/20 bg-primary/5">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          <p className="text-xs font-semibold text-primary/80">Estimated Stitching Delivery</p>
                        </div>
                        {estimateLoading ? (
                          <p className="text-xs text-muted-foreground animate-pulse">Calculating best date...</p>
                        ) : stitchingDeliveryEstimate ? (
                          <p className="text-sm font-bold text-primary">{stitchingDeliveryEstimate}</p>
                        ) : (

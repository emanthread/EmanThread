...
// camelCase → snake_case map (shared with route.ts)
...
// ── GET /api/admin/customer-measurements/[id] ────────────────────────────────
export async function GET(
  ...
  const result = await prisma.$queryRawUnsafe<any[]>(
    `SELECT * FROM "customer_measurements" WHERE "id" = $1 AND "deleted_at" IS NULL LIMIT 1`,
    id
  )

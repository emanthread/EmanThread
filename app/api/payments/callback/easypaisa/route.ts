...
async function handleCallback(req: Request) {
  try {
    ...
    const provider = getProvider("easypaisa");
    ...
    if (!isSandbox && !result.success && result.failureReason?.includes("HMAC")) {
      console.error("[SECURITY] Easypaisa callback HMAC verification failed in production", {
    ...
    if (!transaction) {
      console.error("Easypaisa callback: transaction not found for ref", txnRef);

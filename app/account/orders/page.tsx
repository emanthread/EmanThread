...
import { ArrowLeft, Package, Truck, CheckCircle, Clock, Eye, RefreshCcw, Loader2, MessageCircle, AlertTriangle, Ruler, ChevronDown, ChevronUp } from "lucide-react";
...
export default function OrdersPage() {
  ...
  const [openMeasurementOrderId, setOpenMeasurementOrderId] = useState<string | null>(null);
                ...
                return (
                  <Card key={order.id} className="overflow-hidden">
                    ...
                    <CardContent className="py-4">
                      ...
                      {/* Measurement Details — only shown when this order has a saved measurement snapshot */}
                      {order.measurements && order.measurements.length > 0 && (
                        <div className="mb-4 rounded-lg border border-border/60 bg-muted/30 overflow-hidden">
                          <button
                            ...
                            onClick={() =>
                              setOpenMeasurementOrderId(
                                openMeasurementOrderId === order.id ? null : order.id
                              )
                            }
                          >
                            <span className="flex items-center gap-2">
                              ...
                              Measurement Details
                              {order.measurements.length > 1 && (
                                <span className="text-xs text-muted-foreground font-normal">
                                  ({order.measurements.length} items)
                                </span>
                              )}
                            </span>
                            {openMeasurementOrderId === order.id ? (
                                ...
                                <div key={m.id} className="p-4">
                                  ...
                                  <TailorPrintCard
                                    ...
                                    data={{
                                      ...
                                      stylingPrefs: m.snapshot?.stylingPrefs || null,

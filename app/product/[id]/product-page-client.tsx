...
function ProductDetails({ product }: { product: Product }) {
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
      ...
      {/* Texture Inspection Modal — Full Screen */}
      {textureModalOpen && (
        <div
          className="fixed inset-0 z-50 bg-black/95 flex flex-col"
          onClick={() => setTextureModalOpen(false)}
        >
          {/* Top Bar */}
          <div className="flex items-center justify-between px-6 py-4 text-white">
            <div>
              <h3 className="font-semibold text-sm">🔍 Fabric Texture Inspector</h3>
              <p className="text-xs text-white/60">{product.name} — {product.fabricType}</p>
            </div>
            <button
              onClick={() => setTextureModalOpen(false)}
              className="h-10 w-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-xl"
            >
              ✕
            </button>
          </div>
          ...
          >
            ...
            {showMagnifier && (
              <div
                className="absolute w-48 h-48 rounded-full border-2 border-white/50 pointer-events-none overflow-hidden shadow-2xl"
                style={{
              ...
              >
                <img
                  ...
                  style={{
                    width: "600%",
                    height: "600%",
                    left: `${-magnifierPos.x * 6 + 240}px`,
                    top: `${-magnifierPos.y * 6 + 240}px`,
                  }}
                />
              </div>
            )}
          ...
          {/* Bottom: Thumbnails + Texture Info */}
          <div className="flex items-center gap-6 px-6 py-4 bg-black/50">
            <div className="flex gap-2">
              {product.images.map((img, i) => (
                <button
                  key={i}
                  onClick={() => setSelectedImage(i)}
                  className={`w-14 h-16 rounded-lg overflow-hidden border-2 shrink-0 ${
            ...
            <div className="text-xs text-white/60 leading-relaxed">
              <p className="font-medium text-white/90 mb-1">Fabric Details</p>
              <p>• Type: {product.fabricType}</p>
              <p>• Hover over image for 3x macro zoom</p>
